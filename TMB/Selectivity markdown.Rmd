...
$$RE_{M(X),age} = \sum_p^{N_p} \left(Sel_{M(X)_{(y-p)},age,y-p+1} - Sel_{M(X)_{(y-p+1)}, age,y-p+1}\right) / Sel_{M(X)_{(y-p+1)}, age,y-p+1} / N_p $$
$$MSE_{M(X),age} = \sum_p^{N_p} \left(Sel_{M(X)_{(y-p)}, age,y-p+1} - Sel_{M(X)_{(y-p+1)}, age,y-p+1}\right)^2 / N_p $$
where $RE_{M(X),age}$ and $MSE_{M(X),age}$ are the relative error and mean squared error, respectively, for projected selectivity-at-age from model $M(X)$ (Table 1), $Sel_{M(X)_{(y-p)},age,y-p+1}$ is the one-year projected selectivity from a model fit using data until year $y-p$ ($M(X)_{(y-p)}$), $Sel_{M(X)_{(y-p+1)}, age,y-p+1}$ is the estimated selectivity from model fit using data until year $y-p+1$ ($M(X)_{(y-p+1)}$),and $N_p$ is the number of retrospective peels. Note, that Francis weights were not updated for each retrospective peel. The above metrics were also calculated where projected selectivity $Sel_{M(X)_{(y-p)},age,y-p+1}$ was replaced with the average age-specific selectivity from the last five years, as is currently done for setting reference points. Mohn's Rho what also calculated for spawning stock biomass across peels for each model.
...
Overall, the 3D-AR1 model had the lowest retrospective bias in spawning stock biomass (Figure 10). When evaluating retrospective bias of age-specific selectivity, it is clear that across models 1, 7, and 8, using the projected selectivity outperforms using the average selectivity from the last five years of the assessment (Table 3). However, age-specific retrospective bias varied across selectivity functions. For example, projected selectivity from Model 8 had lower mean squared error for ages 1-3 and 6-9 than projected selectivity from Model 1, but much higher mean squared error for age-10+.
...
![**Figure 10.** Retrospective spawning stock biomass for the four models with n = 5 peels](Results/Figure10_retrospective_ssb.png){#fig:figure10}
